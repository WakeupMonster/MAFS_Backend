...
module.exports.uploadIDDocument = async (req, res) => {
  try {
    ...
    if (!files || !files.front) {
      return res
        .status(400)
        .json({ success: false, message: "ID front image is required" });
    }
    ...
    if (
      ...
      (profile.kyc && profile.kyc.selfie && profile.kyc.selfie.url)
    ) {
      profile.verification.status = "pending";
    ...
    return res
      .status(500)
      .json({ success: false, code: "UPLOAD_FAILED", message: err.message });
  }
};
// ========================================
// 6. UPDATE LOCATION
...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    const [swipeAction, blockStatus] = await Promise.all([
      swipeModel
        .findOne({ swiperId: viewer._id, targetId: targetUserId })
        .lean(),
      Block.findOne({
        $or: [
    ...
    if (blockStatus) {
      return res
        .status(403)
        .json({ success: false, message: "Profile is private or unavailable" });
    }

    // 4. Universal Formatter Call
    const formattedData = await formatPublicProfile(
      viewer,
      targetProfile,
      swipeAction
    );
