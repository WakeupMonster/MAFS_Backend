...
const ProfileSchema = new mongoose.Schema(
  {
    ...
    kyc: {
      ...
      idDocument: {
        type: {
          type: String,
          enum: ["driving_license", "passport", "proof_of_age"],
        },
        frontUrl: String,
...
ProfileSchema.methods.getNextStep = function () {
  ...
  const MANDATORY_STEPS = [
    ...
    {
      key: "relationshipGoalSet",
      screen: "relationship_goals",
      message: "What are you looking for?",
    },
    {
      key: "genderPreferenceSet",
      screen: "gender_preference",
      message: "Who do you want to meet?",
    },
    { key: "ageRangeSet", screen: "age_range", message: "Set age range" },
    {
      key: "distanceRangeSet",
      screen: "distance_range",
  ...
  if (this.kyc.status === "approved" && this.isMandatoryComplete) {
    ...
    const OPTIONAL_STEPS = [
      ...
      { key: "lifestyleSet", screen: "lifestyle", message: "Share lifestyle" },
      {
        key: "languagesSet",
        screen: "languages",
        message: "Languages you know",
      },
      { key: "educationSet", screen: "education", message: "Education" },
      {
        key: "communicationStyleSet",
        screen: "communication",
        message: "Communication style",
      },
      {
        key: "musicPreferenceSet",
        screen: "music",
        message: "Music preference",
      },
...
ProfileSchema.pre("save", function (next) {
  ...
  this.onboardingProgress.relationshipGoalSet =
    this.relationshipGoal?.length > 0;
  this.onboardingProgress.genderPreferenceSet =
    this.preferences?.genderPreference?.length > 0;
  this.onboardingProgress.ageRangeSet = Boolean(
    this.preferences?.ageRange?.min
  );
  this.onboardingProgress.distanceRangeSet = Boolean(
    this.preferences?.distanceRange
  );
  ...
  this.onboardingProgress.idDocumentUploaded = Boolean(
    this.kyc?.idDocument?.frontUrl
  );
  this.onboardingProgress.locationSet = Boolean(
    this.location?.coordinates?.[0]
  );

  // Tier 2 (Optional)
  this.onboardingProgress.bioSet = Boolean(this.bio);
  this.onboardingProgress.lifestyleSet = Boolean(
    this.lifestyle?.religiousBeliefs || this.lifestyle?.zodiacSign
  );
  ...
  this.onboardingProgress.communicationStyleSet = Boolean(
