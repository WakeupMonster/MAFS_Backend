 ...
 * fix_cms_html.js
 * 
 * Rebuilds Privacy Policy and Terms & Conditions HTML from the structured
 * constants (using div-based layout instead of ul/li) and saves to MongoDB.
 * 
 * Run: node scripts/fix_cms_html.js
 */
...
// ── HTML generation helpers (same as termsDataConverter.js) ─────────────
...
const toAlpha = (index) => String.fromCharCode(97 + index);
...
const renderSubList = (subList, depth = 1) => {
  ...
  let html = `<div style="padding-left:${paddingLeft}px;margin:4px 0;display:block;">`;
  subList.items.forEach((item, idx) => {
    const marker = getMarker(subList.listType, idx);
    const text = typeof item === "string" ? item : (item?.text || "");
    html += `<div style="margin-bottom:6px;line-height:1.6;width:auto;max-width:100%;word-break:normal;overflow-wrap:break-word;display:block;">` +
      `<span style="font-weight:600;color:#475569;margin-right:8px;display:inline-block;">${marker}</span>` +
      `<span>${text}</span>`;
...
const sectionContentToHtml = (contentArray) => {
  if (!contentArray?.length) return "";
  let html = "";
  contentArray.forEach((block) => {
    if (block.subtitle) {
      html += `<p style="width:auto;max-width:100%;word-break:normal;overflow-wrap:break-word;"><strong style="font-size:15px;">${block.subtitle}</strong></p>`;
    }
    if (block.text && !block.list) {
      html += `<p style="margin-bottom:12px;line-height:1.7;text-align:left;width:auto;max-width:100%;word-break:normal;overflow-wrap:break-word;">${block.text}</p>`;
    }
    if (block.list && block.items?.length) {
      html += `<div style="padding-left:0;margin:8px 0;display:block;">`;
      block.items.forEach((item, idx) => {
        const marker = getMarker(block.listType, idx);
        const text = typeof item === "string" ? item : (item?.text || "");
        html += `<div style="margin-bottom:10px;line-height:1.6;width:auto;max-width:100%;word-break:normal;overflow-wrap:break-word;display:block;">` +
          `<span style="font-weight:600;color:#475569;margin-right:8px;display:inline-block;">${marker}</span>` +
          `<span>${text}</span>`;
        if (typeof item === "object" && item?.subList) {
...
const sectionsToFullHtml = (sections) => {
  return sections
    .map((section) =>
...
const privacyContents = [
  {
    ...
    content: [
      ...
      { text: `<strong>Personal Information</strong> means information or an opinion about an identified individual or an individual who is reasonably identifiable:` },
      { list: true, listType: "alpha", items: ["whether the information or opinion is true or not; and", "whether the information or opinion is recorded in a material form or not."] },
      ...
      { text: `<strong>Sensitive Information</strong> is defined in the Privacy Act as including information or opinion about such things as an individual's racial or ethnic origin, political opinions, membership of a political association, religious or philosophical beliefs, membership of a trade union or other professional body, criminal record or health information.` },
      { text: `The Information which we collect and hold about you may include the following categories:` },
      { subtitle: `Personal Information`, text: `We collect the following Personal Information:` },
      { list: true, listType: "disc", items: ["Name", "Date of birth", "Email address", "Phone number", "Login credentials", "Gender", "Profile information for filtering purposes"] },
      ...
      { subtitle: `ID Verification`, text: `By completing ID verification, you consent to us using and storing information that includes uploading a photo. We only use this information for confirming your identity and preventing unauthorised accounts.` },
      ...
      { subtitle: `Payment Information`, text: `We will not store or process any card or payment details within our system. All payments will be handled securely by third-party providers, specifically Apple App Store and Google Play.` },
      ...
      { subtitle: `Usage Data`, text: `This includes information such as when users log in, which screens or features they interact with, swiping behaviour, match activity and overall engagement patterns.` },
      ...
      { subtitle: `<i><strong>Technical Data</strong></i>`, text: `<i>We will collect technical information automatically through 3rd party tools.</i>` },
      ...
      { subtitle: `Cookies (Landing Page / Website Only)`, text: `Cookies will be used on the website only, not within the mobile app.` },
      { text: `The landing page will use essential cookies required for basic site functionality.` },
      { subtitle: `Marketing Preferences`, text: `We will include a simple opt-in and opt-out system for marketing communications. Users will be able to unsubscribe or update their preferences either through their account settings or directly through email links.` },
      ...
      { subtitle: `<strong>Giveaways/Promotions</strong>`, text: `<i>If you are the recipient of any rewards or giveaways within the app, we will collect and store data related to that giveaway for auditing purposes, i.e. name, prize, date it was awarded and to what email address it was sent to.</i>` },
  ...
  },
  {
    ...
    content: [
      { list: true, listType: "alpha", items: [
  ...
  },
  {
    ...
    content: [
      { list: true, listType: "alpha", items: [
        { text: "We store your Personal Information in a way that reasonably protects it from unauthorised access, misuse, modification or disclosure. When we no longer require your Personal Information for the purpose for which we obtained in, we will take reasonable steps to destroy and anonymise or de-identify it. Most of the Personal Information that is stored in our client files and records will be kept for a maximum of seven (7) years or for as long as legally necessary, to fulfill our legal record keeping obligations.", subList: { listType: "roman", items: ["The app is built with security measures to prevent people from taking screenshots of your profile. However, by using the app you acknowledge and accept that people may still access your photos through other means."] } },
        { text: "The Australian Privacy Principles:", subList: { listType: "roman", items: ["permit you to obtain access to the Personal Information we hold about you in certain circumstances (Australian Privacy Principle 12); and", "allow you to correct inaccurate Personal Information subject to certain exceptions (Australian Privacy Principle 13)."] } },
        { text: "You may also request the deletion of your personal information by contacting us using the details below. We will take reasonable steps to delete your information from our systems, except where we are required to retain certain records to comply with our legal obligations.", subList: { listType: "roman", items: ["Note that we can only delete data from our own systems. If you wish to delete data held by third-party service providers (such as Apple or Google), you will need to contact them directly through their respective privacy request processes."] } },
...
async function main() {
  ...
  console.log("\n📝 Rebuilding Privacy Policy HTML...");
  const ppSections = privacyContents.map((s) => ({
    id: s.id,
    iconName: s.iconName,
    title: s.title,
    html: sectionContentToHtml(s.content),
  }));
  const ppHtml = sectionsToFullHtml(ppSections);

  const ppResult = await PrivacyPolicy.findOneAndUpdate(
    {},
    { title: "Privacy Policy", description: ppHtml },
    { new: true, upsert: true }
  );
  console.log("✅ Privacy Policy updated:", ppResult._id);

  console.log("\nDone! Both documents updated with clean div-based HTML.");
