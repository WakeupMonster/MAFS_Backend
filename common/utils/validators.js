...
const validateDiscoveryFilters = (req, res, next) => {
  const { discoveryFilters } = req.body; // ─── 1. discoveryFilters must exist ───

  if (
    !discoveryFilters ||
    typeof discoveryFilters !== "object" ||
    Array.isArray(discoveryFilters)
  ) {
    return sendError(res, "discoveryFilters must be a valid object.");
  } // ─── 2. Unknown top-level keys ─── // ─── 3. relationshipGoal must be string only ───
  ...
  if (discoveryFilters.advanced !== undefined) {
    ...
    if (unknownKeys.length > 0) {
      return sendError(
        res,
        `Unknown key in advanced filters: ${unknownKeys[0]}`
      );
    }
